# From idea to LOTUSim contribution — faster with AI agents

Slides d'un talk sur l'utilisation d'agents de code IA pour contribuer à [LOTUSim](https://github.com/LOTUSRobotics) (simulateur robotique sous-marin), de la lecture du codebase à la pull request.

## Voir la présentation

https://cmoron.github.io/lotusim-agents-talk/

## Navigation

| Touche | Action |
|---|---|
| `→` · `Space` · `PageDown` | Slide suivante |
| `←` · `PageUp` | Slide précédente |

## Local

Aucun build, aucune dépendance — c'est un fichier HTML autonome.

```bash
# Servir localement
python -m http.server 8000
# puis ouvrir http://localhost:8000
```

Ou ouvrir directement `index.html` dans un navigateur.

## Déploiement

Servi par GitHub Pages depuis `main` / `/` à chaque push.
